# GPTQ implementation (from Mixture-Compressor-MoE), imports from local utils
import math
import time
import logging
...
class GPTQ:
    ...
    def __init__(self, layer, logger_instance, name, wbits):
        self.layer = layer
    ...
    def print_loss(self, name, q_weight, weight_error, timecost, modules=None, bit=3):
        ...
        if self.inp1 is not None:
            q_obj = quantizer.Quantizer()
            q_obj.configure(8, perchannel=False, sym=True, mse=False)
            q_obj.find_params(self.inp1)
            q_in = q_obj.quantize(self.inp1).type(torch.float16)
            ...
            q_SNR = "-"
            fp_SNR = "-"
        self.logger.info("%s weight_err=%.6f fp_SNR=%s q_SNR=%s time=%.2fs", name[:40], weight_error, fp_SNR, q_SNR, timecost)
    ...
    def static_fasterquant(self, blocksize=128, percdamp=.01, groupsize=-1, actorder=False, name=''):
        ...
        try:
            ...
            logger.warning("Cholesky failed: %s", e)
